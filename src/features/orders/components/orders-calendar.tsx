...
/** Chips visibles por celda (huella fija); el resto vía “+N casos” o clic en el día */
...
function useSquareCalendarLayout(
  ...
  useEffect(() => {
    function update() {
      ...
      if (w < 768) {
        setLayout({ isMobile: true, height: "auto" });
        return;
      }
      ...
      const gap = 6;
      // Compacto pero con espacio real para 2 pastillas (22px) + número + padding
      const raw = (width - gap * 8) / 7;
      const cellSide = Math.min(122, Math.max(102, Math.floor(raw)));
      const weeks = 6;
      const chrome = 120;
...
export function OrdersCalendar({ events }: Props) {
  ...
  function onDateClick(info: DateClickArg) {
    // Secondary: day overflow list — primary interaction is the case chip
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-white/50 bg-white/70 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl transition-all duration-300 ease-in-out">
      <div className="border-b border-white/40 bg-gradient-to-r from-white/80 via-white/60 to-secondary/20 px-4 py-3.5 sm:px-5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {isMobile
                ? "Lista en móvil · toca un caso para abrir su tarjeta"
      ...
      <div ref={containerRef} className="d1-calendar w-full p-2 sm:p-3 lg:p-4">
        <FullCalendar
          ref={calendarRef}
